...
cursor = con.cursor()
database = "create database if not exists e_commerce"
cursor.execute(database)
cursor.execute('use e_commerce')
cursor.execute('create table if not exists data(id INT,name VARCHAR(100),phone FLOAT(9,2),descr VARCHAR(100))')
...
def insert():
    ...
    if (id == "" or name == "" or phone == "" or desc == ""):
        ...
        if prod_exist:
            MessageBox.showinfo("Insert Status ", "ID is already existed")
        else:
...
def delete():
    ...
    if (e_id.get() == ""):
        ...
        if prod_exist:
            ...
            MessageBox.showinfo("Insert Status ", "ID does not exist")
...
def admin_page(root):
    ...
    #get1 = Button(root, text='Get', font=('italic', 10), bg='white', command=show)
    #get1.place(x=260, y=150)
    ...
    global list
    list = Listbox(root)
    list.place(x=400, y=60, width=380)
    show()
