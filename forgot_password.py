...
def printt(root):
    btn_otp = Button(root, text="Validate User", width=20, bg="orange", fg="white", command=lambda :check_user(root)).place(x=790, y=280)
...
def check_user(root):
    ...
    if data:
        ...
        btn_otp = Button(root, text="Get OTP", width=20, bg="orange", fg="white", command=lambda :otp_generate(root)).place(x=790, y=410)
        ...
        btn_otp = Button(root, text="Change Password", width=20, bg="orange", fg="white", command=lambda :valid(root)).place(x=790, y=490)
...
def valid(root):
    ...
    if password != None and  confirm_password != None and entry_otp != None:
        ...
        messagebox.showinfo("Error", "Please check the fields")
...
def forgot_start(root):
    ...
    back_to_login = Button(root, text="Back", width=20, bg="orange", fg="white", command=lambda :forgot_to_login(root)).place(x=1000, y=160)
