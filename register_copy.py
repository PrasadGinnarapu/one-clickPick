...
def register(root):
    ...
    if user_exist:
        ...
        messagebox.showinfo(title="InValid", message=f"{firstname} already existed user. Try login")
        fname_entry.delete(0, END)
        lname_entry.delete(0, END)
        email_entry.delete(0, END)
        passwrd_entry.delete(0, END)
...
def reg_start(root):
    ...
    # but_quit = Button(root,text="Quit",width=12,bg="green",fg="white",command=exitt).place(x=810,y=450)
